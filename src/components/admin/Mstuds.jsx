...
const Students = ({ cont, user, setUser, recent }) => {
  ...
  }, [trans, hidee, recent, currentPage, user, cont, checkCalled]);
  ...
  const Search = async (tid) => {
    ...
    if (tid.length > 2) {
      ...
      async function Goon() {
        try {
          await axios
            .get(`${cont.api}admin/msearchstuds/${tid}`, {
              headers: { userid: user.userid },
            })
            .then((res) => {
              if (res.data.success) {
                  ...
                  <div>
                    <div className="d-flex jbtw col-md-6 mx-auto">
                      <div className="w-100 jbtw">
                        <Talk
                          bg={true}
                          a={`Codarbrain is yet to get a student`}
                          b={`${month} will pop up here as soon as you register students this month.`}
                          c={`Go to batch in focus to manually add student/transaction .`}
