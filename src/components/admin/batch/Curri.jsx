...
const Curri = ({ cont, user, setUser }) => {
  ...
  const Move = ()=>{
    ...
    if(batches && batches.length > 0){
      ...
      Swal.fire({title: `Error:There no other course batches to move student to.`,
  ...
  return (
    <div className="px-1 app-container">
      ...
      <Offcanvas show={showstud} onHide={handleClosestud}>
        <Offcanvas.Body>
          <Createstud course={course} setCourse={setCourse}
        ...
        >
          ...
          {student ? (
            ...
            <TabPanel header={
              ...
              {(student.edited || student.moved) && user.host ?(<div className="mt-3 mb-2">
                <h2 className="text-center">Technical <i className="fa fa-gear"></i> made on {student.name}</h2>

                <div className="px-2">
                  {student.moved ? (<h4 className="text-start mb-2"> <i className="fa fa-list"></i>  Summary :{student.movequery}</h4>):""}

                  {student.edited ? (<h4 className="text-start mb-5"> <i className="fa fa-list"></i>  Summary :{student.editquery}</h4>):""}
                  ...
                  {!amount ? (
                    ...
                    <div className="row">
                      <Card className={`card rounded bg-primary pt-2`}>
                        ...
                        <Card.Body>
                          {user.allowinstallments ?(<Card.Title>Remember {studentb.name} </Card.Title>):
                          (<Card.Title>Complete {studentb.name}'s payment </Card.Title>)}
                          {user.allowinstallments ?
                            (
                              <div>
                                <h2 className="text-light"> is in debt of{" "}
                                  <h2 className="text-danger d-inline">
                                    {studentb.dowe}
                                  </h2>
                                </h2>
                                <div className="p-inputgroup flex-1 col-md-6 col-lg-6 col-10 mx-auto">
                                  <span className="p-inputgroup-addon fs-2">₦</span>
                                  <InputNumber
