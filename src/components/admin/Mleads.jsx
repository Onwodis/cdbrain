...
const Mleads = ({ cont, user, setUser, recent }) => {
  ...
  useEffect(() => {
    const Check = async (page = 1) => {
      try {
        await axios
          .get(
            `${cont.api}admin/recentleads?page=${page}&limit=${pageSize}}`,
            {
              headers: { userid: user.userid },
            }
          )
  ...
  const Search = async (tid) => {
    ...
    if (tid.length > 2) {
      ...
      async function Goon() {
        try {
          await axios
            .get(`${cont.api}admin/msearchleads/${tid}`, {
              headers: { userid: user.userid },
            })
            .then((res) => {
              if (res.data.success) {
  ...
  const preptrans = (lid) => {
    ...
    if (ng) {
      setLeadp((prev) => ({ ...prev, ...ng, amount: ng.dcfee }));
                          ...
                          {trans.map(
                            (
                              ...
                              >
                                ...
                                <td className="text-center">
                                  <Cttb id="tooltip-cart" tooltip={`${dcfee}`}>
                                    <div>{cname}</div>
                                  </Cttb>
                                </td>
                                {/* <td className="text-center">{lid}</td> */}
