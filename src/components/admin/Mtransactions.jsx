...
const Transactions = ({ cont, user, setUser, recent }) => {
  ...
  const Pago = async (page) => {
    try {
      await axios
        .get(
          `${cont.api}admin/recenttransactions?page=${page}&limit=${pageSize}}`,
          {
  ...
  const Search = async (tid) => {
    ...
    if (tid.length > 2) {
      ...
      async function Goon() {
        try {
          await axios
            .get(`${cont.api}admin/msearchtrans/${tid}`, {
              headers: { userid: user.userid },
            })
            .then((res) => {
              if (res.data.success) {
                              ...
                              >
                                ...
                                >
                                  <Cttb id="tooltip-cart" tooltip={`${other}`}>
                                    <h5 className="ss3">{name}</h5>
                                  </Cttb>
                                </td>
                                <td className="ssj3">{branch}</td>
                                ...
                                <td className="text-center ss3">{batch}</td>
                                <td className="text-center ss3">{reference}</td>
                                <td className="text-center">{type}</td>
                                <td>{orderb}</td>

                                <td>{damount}</td>
                                <td className="ss3">{paidby}</td>
                                <td className="ss3">{approvedby}</td>
                                {user.host ? (
                                  <td
                                    className="text-center cpp"
