...
const Admins = ({ cont, user, setUser }) => {
  ...
  const [mentors, seadmins] = useState();
  ...
  useEffect(() => {
    const Check = async () => {
      try {
        await axios
          .get(`${cont.api}admin/getadmins`,{headers:{userid:user.userid}})
          .then((res) => {
            if (!res.data.nolicense){
              if (res.data.success) {
                ...
                if (res.data.admins) {
                    // alert('kjhgfc');
                  setAdmins((prev) => [...res.data.admins]);
  ...
  const Newadmin = async (e) => {
    ...
    try {
      ...
      await axios
        .post(`${cont.api}admin/addadmin`, formDataToSend,{headers:{userid:user.userid}})
        .then((res) => {
          if (res.data.success) {
            if (res.data.exist) {
              Swal.fire({
  ...
  const EditAdmin = async (e) => {
    ...
    try {
      ...
      await axios
        .post(`${cont.api}admin/editadmin`, formDataToSend,{headers:{userid:user.userid}})
        .then((res) => {
          if (res.data.success) {
            if (res.data.exist) {
              Swal.fire({
  ...
  const Del = async (tid, name) => {
    // alert(`${cont.api}admin/deladmin/${tid}`);
    async function Goon() {
      try {
        await axios.get(`${cont.api}admin/deladmin/${tid}`,{headers:{userid:user.userid}}).then((res) => {
          if (res.data.success) {
            // setFirst(false);
            ...
            if (res.data.admins) {
              ...
              Swal.fire({
                title: "Deleted !",
                text: "admins account  deleted successfully ! !",
                icon: "success",
              });
            }
          } else {
    ...
    Swal.fire({
      title: `Are you sure you want to delete ${name}'s (admins account) ?`,
  ...
  const Access = async (tid, name) => {
    ...
    async function Goon() {
      try {
        await axios
          .get(`${cont.api}admin/access?userid=${tid}&fromid=${user.userid}`,{headers:{userid:user.userid}})
          .then((res) => {
            if (res.data.success) {
              // setFirst(false);
  ...
  const License = async (tid, name) => {
    ...
    async function Goon() {
      try {
        await axios
          .get(`${cont.api}admin/license?userid=${tid}&fromid=${user.userid}`,{headers:{userid:user.userid}})
          .then((res) => {
            if (res.data.success) {
              // setFirst(false);
                    ...
                    <div class="formbg-outer ">
                      <div class="formbg ">
                        <div class="formbg-inner ">
                          <h2 class="padding-bottom--15">Add new admin</h2>
                      ...
                      <thead style={{ zIndex: 2 }} className="tstick">
                        ...
                        >
                          ...
                          <th>
                            ...
                            <Cttb
                              id="tooltip-cart"
                              tooltip={`What is Cud access ? <br>Cud is the access given to slave admin (not host admin) to perform complete crud operations on other elements on codarbrain as well as admin , only host admin can give cudaccess`}
                            >
                        ...
                        {admins.map(
                          (
                            ...
                            el
                              ...
                              <tr key={userid} style={{ position: "relative" }} className="mtr">
                                <td>{index + 1}</td>
                                ...
                                <td>
                                  <Ctt
                                    ...
                                    tooltip={`${
                                      cudaccess
                                        ? `GRANTED :this means that ${name}  can PERform crud operations on this system just as the host admin from ${gender === "male" ? `his` : `her`} codar brain account`
                                        : `Un-GRANTED :this means that ${name}  can PERform crud operations on system just as the host admin from ${gender === "male" ? `his` : `her`} codar brain account`
                                    } `}
                                  >
                                    {cudaccess ? (
                                      <div
                                        onClick={(e) => Access(userid, name)}
                                ...
                                <td
                                  title={
                                    licensed ? `Active` : `License expired`
                                  }
                                  className={`${
                                ...
                                >
                                  {user.cudaccess ? (
                                    <Ctt
                                      ...
                                      tooltip={`${
                                        licensed
                                          ? `Licensed :this means that ${name}  can access ${gender === "male" ? `his` : `her`} platform ,click to toggle license`
                                          : `Licensed :this means that ${name}  can not access ${gender === "male" ? `his` : `her`} platform  ,click to toggle license`
                                      } `}
                                    >
                                      {licensed ? (
                                        <div
                                    ...
                                    <Ctt
                                      ...
                                      tooltip={`${
                                        licensed
                                          ? `Licensed :this means that ${name}  can access ${gender === "male" ? `his` : `her`} ,you can not invoke or revoke license`
                                          : `Licensed :this means that ${name}  can not access ${gender === "male" ? `his` : `her`} ,you can not invoke or revoke license`
                                      } `}
                                    >
                                      {licensed ? (
                    ...
                    <div className="d-flex jbtw col-md-6 mx-auto">
                      <div className="w-100 jbtw">
                        ...
                        <Talk
                          ...
                          b={`You currently do not have any admin `}
                          c={`pls click the button above to add admin(s) .`}
